...
as $$
  with app_user as (
    ...
    where u.supabase_auth_user_id = p_supabase_auth_user_id
      and private.current_auth_user_id() = p_supabase_auth_user_id
  ),
  platform_context as (
    select
      u.id as user_id,
...
revoke all on function private.current_auth_user_id() from public;
revoke all on function private.resolve_auth_context(uuid) from public;

grant execute on function private.current_auth_user_id() to app_runtime, app_readonly;
