            ...
            {selected ? (
              <dl className="mt-5 grid gap-4 text-sm">
                ...
                <div>
                  ...
                  <dd className="mt-1">
                    {selected.ipAddress || "Not recorded"} ·{" "}
      ...
      >
        ...
        <Card className="super-admin-surface">
          ...
          <CardContent className="flex items-center justify-end gap-2 pt-0">
            {saveConfiguration.isPending ? <PlatformConfigurationMatrixLoader /> : null}
            <Button type="submit" disabled={saveConfiguration.isPending}>
              Publish platform configuration
            </Button>
          </CardContent>
        </Card>
      </form>
...
function PlatformConfigurationMatrixLoader() {
  return (
    <div className="platform-configuration-matrix-loader" role="status" aria-label="Publishing platform configuration">
      {Array.from({ length: 8 }, (_, index) => <span key={index} className="platform-configuration-matrix-loader__digit">{index % 2}</span>)}
      <span className="platform-configuration-matrix-loader__glow" aria-hidden="true" />
    </div>
  );
}
