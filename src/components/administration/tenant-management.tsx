      ...
      >
        ...
        <Card>
          ...
          <CardContent className="grid gap-5 md:grid-cols-2">
            ...
            <fieldset className="md:col-span-2"><legend className="text-sm font-medium">Enabled modules</legend><div className="mt-2 grid gap-2 sm:grid-cols-3">{["dashboard", "clients", "tasks", "work-groups", "employees", "managers", "documents", "invoices", "reports"].map((module) => <label key={module} className="flex items-center gap-2 text-sm"><input type="checkbox" value={module} {...form.register("modules")} />{module.replace("-", " ")}</label>)}</div>{form.formState.errors.modules ? <p className="mt-1 text-xs text-danger">{form.formState.errors.modules.message}</p> : null}</fieldset>
          </CardContent>
        </Card>
        <Card>
          <CardHeader>
