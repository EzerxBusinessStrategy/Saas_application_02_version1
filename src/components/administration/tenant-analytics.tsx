...
export function TenantAnalyticsPage() {
  ...
  const [draftFilters, setDraftFilters] = useState<TenantAnalyticsFilters>({});
  ...
  const updateDraft = (next: Partial<TenantAnalyticsFilters>) => setDraftFilters((current) => ({ ...current, ...next }));
  const filtersDirty = !sameFilters(filters, draftFilters);
  const incompleteDateRange = Boolean(draftFilters.from) !== Boolean(draftFilters.to);
  const invalidDateRange = Boolean(draftFilters.from && draftFilters.to && draftFilters.from > draftFilters.to);
  const updateAnalytics = () => {
    if (incompleteDateRange || invalidDateRange) return;
    setFilters({ ...draftFilters, financialYearId: draftFilters.from && draftFilters.to ? undefined : draftFilters.financialYearId });
  };
...
function sameFilters(left: TenantAnalyticsFilters, right: TenantAnalyticsFilters) { return left.tenantId === right.tenantId && left.financialYearId === right.financialYearId && left.from === right.from && left.to === right.to; }
function AnalyticsUpdateLoader() { return <div className="analytics-update-loader" role="status" aria-label="Updating analytics"><span className="analytics-update-loader__box analytics-update-loader__box--one" /><span className="analytics-update-loader__box analytics-update-loader__box--two" /><span className="analytics-update-loader__box analytics-update-loader__box--three" /></div>; }
