...
export function TenantAnalyticsPage() {
  ...
  return <div className="super-admin-portal flex flex-col gap-[30px]">
    ...
    <Card className="super-admin-surface"><CardContent className="grid gap-4 p-[30px] md:grid-cols-4">
      ...
      <div className="md:col-span-4 flex items-center gap-3"><Button type="button" disabled={!from || !to || from > to} onClick={() => set({ from, to, financialYearId: undefined })}>Apply date range</Button>{(from || to) && <p className="text-sm text-muted-foreground">Choose both dates to apply a custom period.</p>}</div>
    </CardContent></Card>
