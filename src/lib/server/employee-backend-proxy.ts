...
export async function proxyEmployeeBackend({
  path,
  init,
  unauthenticatedMessage = "Employee session required.",
  unavailableMessage = "Employee service unavailable.",
  ...
  try {
    ...
    if (refreshed && backendResponse.status !== 401) {
      setSuperAdminSessionCookies(response, refreshed, rememberMe, "employee");
    }
